    ...
    var funs = {
      ...
      "SAVE_OPTION": function(){
        ...
        $.post("/sql", o, function(D){
          ...
          if(D.ID_OPTION.length == 0){
            query = $.sprintf(
              "INSERT INTO %s (name,dbuser,access,fun,sign,val) "+
              "VALUES(?,?,?,?,?,?)", optInp.SQL_SysTable);
            para = [
              "",                // name
              optInp.User,       // user
              optInp.User,       // access
            ...
            query = $.sprintf(
              "REPLACE INTO %s (id,name,dbuser,access,fun,sign,val) "+
              "VALUES(?,?,?,?,?,?,?)", optInp.SQL_SysTable);
            para = [
              D.ID_OPTION[0].id, // id
              "",                // name
      ...
      "FILTER_LIST": function(){
        var o = {
          ...
          cmd:  [{
            sgn:   "FILTER_LIST",
            query: $.sprintf(
              "SELECT * FROM %s WHERE fun='FILTER' AND sign=? AND (dbuser=? OR access=? OR access='' OR access RLIKE(?)  OR access RLIKE(?) OR access RLIKE(?)) ORDER BY name",
              optInp.SQL_SysTable
            ),
            para: [
              optInp.Sign,      // navig identification
      ...
      "TABLE_INFO": function(){
        var o = {
          ...
          cmd:  [{
            sgn:   "TABLE_INFO",
            query: "DESCRIBE "+optInp.SQL_Table,
            para: []
          }]
        };
        $.post("/sql", o, function(D){
          var d = D.TABLE_INFO.map(function(x) {
            x = {name: x.Field};
            return x;
          });
      ...
      "UPDATE_LIST": function(){
        var o = {
          ...
          cmd:  [{
            sgn:   "UPDATE_LIST",
            query: $.sprintf(
              "SELECT * FROM %s WHERE fun='UPDATE' AND sign=? AND (dbuser=? OR access=? OR access='' OR access RLIKE(?)  OR access RLIKE(?) OR access RLIKE(?)) ORDER BY name",
              optInp.SQL_SysTable
            ),
            para: [
              optInp.Sign,      // navig identification
      ...
      "REPORT_LIST": function(){
        var o = {
          ...
          cmd:  [{
            sgn:   "REPORT_LIST",
            query: $.sprintf(
              "SELECT * FROM %s WHERE fun='REPORT' AND sign=? AND (dbuser=? OR access=? OR access='' OR access RLIKE(?)  OR access RLIKE(?) OR access RLIKE(?)) ORDER BY name",
              optInp.SQL_SysTable
            ),
            para: [
              optInp.Sign,      // navig identification
...
*/
      ...
      $.post("/sql", o, function(D){
        if(D === null)
          D = {};
//console.log("o", sgn, o.cmd);console.log(D);//return;
        if(D.TOTAL)
